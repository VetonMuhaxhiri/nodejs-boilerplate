...
EOF
		...
		if [ "$MYSQL_USER" -a "$MYSQL_PASSWORD" ]; then
            ...
            # Make the Grant All for current user to create database outside database
      ...
      if [ "$MYSQL_DATABASE_TESTING" ]; then
        echo "CREATE DATABASE \`"$MYSQL_DATABASE_TESTING"\` ;" | "${mysql[@]}"
      fi
